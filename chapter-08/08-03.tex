...
\]
serves as a convex surrogate for rank \citet{candes2009exact,candes2010power}. The regularised matrix completion estimator solves:
\[
...
\paragraph{Optimal $\lambda$} balances these extremes. Cross-validation provides a data-driven choice: partition the observed cells $\Omega$ into training (80 per cent) and validation (20 per cent) sets; solve the nuclear-norm problem on the training set for a grid of $\lambda$ values (e.g., $\lambda \in \{0.1, 0.5, 1, 2, 5, 10, 20\} \times \sigma_1$, where $\sigma_1$ is the largest singular value of the observed matrix); select the $\lambda$ minimising validation error.
...
\paragraph{Random missingness.} If each cell is observed independently with probability $p$, recovery is feasible when $p \geq C R \log(NT) / \min(N,T)$. For a rank-5 matrix with $N = T = 100$, this requires observing roughly 50 per cent of entries.
