...
\]
where $\alpha_i$ is a unit fixed effect, $\gamma_t$ is a time fixed effect, $\lambda_i \in \mathbb{R}^R$ is the loading vector for unit $i$, $f_t \in \mathbb{R}^R$ is the factor vector for period $t$, and $\varepsilon_{it}$ is an idiosyncratic error with mean zero and finite variance.
...
For the remainder of this section, we work with the demeaned pure factor model:
\[
