...
\begin{quote}
\textbf{Box 5.1: Diagnostic Quick Reference}
...
\textit{Pre-trend checks}: Test joint significance of $\theta_k$ for $k < -1$; plot pre-treatment coefficients; conduct placebo-in-time test. Remember: pre-trend tests have limited power (Roth, 2022)---supplement with visual inspection.

\textit{Support checks}: Report observations and cohorts by event time $k$. Flag thin support ($<30$ obs, single cohort, or effective $N < \sqrt{N}$). Bin extreme event times if needed.
