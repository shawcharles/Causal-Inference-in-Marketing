...
We need to be precise about what panel data means.  The fact that we observe the same individual units over multiple time periods is what distinguishes it and gives it statistical power.  This is not the same as 'repeated cross-sections', which involve drawing different samples of a population at different points in time.  Repeated cross-sections enable us to monitor group averages.  Panel data enables us to account for unobserved individual characteristics.

 This advantage places panel methods in a more general hierarchy of causal reasoning.  Pearl \cite{pearl2009causality} describes three ascending levels.  At the lowest rung, association, we see correlations in data: advertising expenditure covaries with sales.  One step up, intervention, we consider the consequences of actions: what would happen to sales if we increased advertising expenditure by 10\%?  At the summit, we consider alternative histories: what would sales have been in the previous quarter if we had run a different campaign?
 ...
 Moving up Pearl's hierarchy necessitates stronger assumptions.  Association requires only that we observe variables and quantify their relationships.  Intervention necessitates assumptions about which variables are confounders, whether parallel trends exist, and whether a factor structure accurately captures unobserved heterogeneity.  Counterfactuals necessitate even more robust assumptions about the data-generating process's stability in the face of hypothetical changes.  Panel methods make these assumptions explicit, testable where possible, and subject to sensitivity analysis -- an approach that we emphasise throughout the book.
