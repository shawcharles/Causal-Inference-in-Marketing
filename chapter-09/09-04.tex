...
\[
Y_{it}(0) = \underbrace{X_{it}^\top \beta}_{\text{covariate effect}} + \underbrace{\lambda_i^\top f_t}_{\text{low-rank component}} + \varepsilon_{it},
\]
where:
\begin{enumerate}[(i)]
...
\paragraph{Types of covariates.}
\begin{itemize}
\item \textbf{Unit-specific covariates} (time-invariant): Store size, location (urban/rural), demographics (median income, population density), format (supermarket/convenience). Enter as $X_i' \beta$.
\item \textbf{Time-varying covariates}: Weather (temperature, precipitation), holidays, macroeconomic conditions (unemployment, consumer confidence). Enter as $X_{it}' \beta$.
\item \textbf{Network structure}: Geographic proximity, competitive relationships. Captured by graph-regularised methods (below).
\end{itemize}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!50!black,title=Algorithm: Two-Step Covariate-Assisted Matrix Completion]
\textbf{Input:} Outcomes $\{Y_{it}\}$, covariates $\{X_{it}\}$, untreated cells $\Omega$, treated cells $\mathcal{T}$.

\textbf{Step 1: Estimate covariate effect.}
\begin{enumerate}
...
\textbf{Step 2: Matrix completion on residuals.}
\begin{enumerate}
...
\textbf{Step 3: Impute counterfactuals.}
\begin{enumerate}
...
\textbf{Output:} Counterfactuals $\{\hat{Y}_{it}(0)\}_{(i,t) \in \mathcal{T}}$ and treatment effects $\{\hat{\tau}_{it}\}$.
\end{tcolorbox}
...
\begin{enumerate}[(i)]
    ...
    \item \textbf{Treatment independence:} $\mathbb{E}[X_{it} | W_{it} = 1, \lambda_i, f_t] = \mathbb{E}[X_{it} | W_{it} = 0, \lambda_i, f_t]$.
\end{enumerate}
\end{assumption}
...
Several packages support covariate-assisted matrix completion:

\paragraph{Python.}
\begin{itemize}
\item \texttt{fancyimpute}: Matrix completion with support for covariates via feature matrices.
\item \texttt{surprise}: Recommender system library with side information support (for user/item features).
\item Custom implementations using \texttt{scipy.optimize} for the two-step procedure.
\end{itemize}

\paragraph{R.}
\begin{itemize}
\item \texttt{softImpute}: Nuclear-norm regularised MC. Combine with \texttt{lm()} for two-step procedure.
\item \texttt{cmfrec}: Collective matrix factorisation with side information.
\item \texttt{recosystem}: Matrix factorisation for recommender systems with features.
\end{itemize}

\paragraph{Graph-regularised MC.}
\begin{itemize}
\item \texttt{pygsp} (Python): Graph signal processing; combine with MC for graph-regularised estimation.
\item \texttt{igraph} + custom solvers: Build adjacency matrix and implement ADMM.
\end{itemize}
...
\paragraph{Results.}
\begin{itemize}
\item \textbf{Covariate-assisted ATT:} 18\% increase in sales (95\% CI: [14\%, 22\%]).
\item \textbf{Standard MC ATT:} 15\% increase (95\% CI: [10\%, 20\%]).
\item The difference arises because covariate-assisted MC accounts for store heterogeneity.
\end{itemize}

\paragraph{Heterogeneity by store type.}
\begin{itemize}
\item Large stores: 25\% (95\% CI: [19\%, 31\%])
\item Medium stores: 15\% (95\% CI: [10\%, 20\%])
\item Small stores: 10\% (95\% CI: [5\%, 15\%])
\end{itemize}
This heterogeneity is masked by standard MC, which produces a single average effect.
