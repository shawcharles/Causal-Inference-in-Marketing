...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!50!black,title=Algorithm: Gibbs Sampler for Bayesian Matrix Completion]
\textbf{Input:} Observed data $\{Y_{it}\}_{(i,t) \in \Omega}$, maximum rank $R_{\max}$, hyperparameters $(\nu_\lambda, \Psi_\lambda, \nu_f, \Psi_f, a_\sigma, b_\sigma, a_\tau, b_\tau)$, iterations $S$, burn-in $B$.

\textbf{Initialise:} Set $\lambda_i^{(0)}$, $f_t^{(0)}$, $\tau_r^{(0)}$, $\sigma^{2(0)}$ using SVD on observed data.

\textbf{For each iteration $s = 1, \ldots, S$:}
\begin{enumerate}
...
\textbf{Output:} Posterior draws $\{\tau^{(s)}_{\text{ATT}}\}_{s=B+1}^S$, credible intervals, effective rank $\hat{R}_{\text{eff}}$.
\end{tcolorbox}
