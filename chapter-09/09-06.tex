...
\item \textbf{Impute counterfactuals:} For treated cells $(i,t) \in \mathcal{W}$, draw
\[
\hat{Y}_{it}^{(s)}(0) \sim \mathcal{N}(\lambda_i^{(s)\top} f_t^{(s)}, \sigma^{2(s)}).
\]

\item \textbf{Compute treatment effect:} $\tau^{(s)}_{\text{ATT}} = \frac{1}{|\mathcal{W}|} \sum_{(i,t) \in \mathcal{W}} (Y_{it} - \hat{Y}_{it}^{(s)}(0))$.
\end{enumerate}
...
Several software packages support Bayesian matrix completion. In Python, \texttt{pymc} provides general Bayesian modelling with support for custom factor models and ARD priors, \texttt{bpmf} implements Bayesian Probabilistic Matrix Factorisation with a Gibbs sampler for low-rank models, and \texttt{edward2} or \texttt{tensorflow-probability} offer variational inference for large-scale problems. In R, \texttt{blavaan} provides Bayesian latent variable models supporting factor analysis with MCMC, \texttt{brms} offers Bayesian regression models via Stan with the ability to specify custom factor structures, and \texttt{MCMCpack} provides general MCMC samplers including factor model implementations. Stan provides a flexible language for specifying custom Bayesian models, allowing the factor model to be implemented directly with ARD priors, with \texttt{rstan} (R) and \texttt{pystan} (Python) providing interfaces.
...
\paragraph{Credible intervals.} The posterior yields 95\% credible intervals that vary with category size. For the largest category (50 products), the ATT is 15\% with a 95\% credible interval of 12--18\%. For the smallest category (15 products), the ATT is 12\% with a wider 95\% credible interval of 8--16\%, reflecting greater uncertainty due to limited data.
