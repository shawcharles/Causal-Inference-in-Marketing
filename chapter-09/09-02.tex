...
\[
\mathbb{E}[\mathcal{Y}_{ijt}(0) | (i,j,t) \in \mathcal{W}] = \mathbb{E}[\mathcal{Y}_{ijt}(0) | (i,j,t) \in \Omega],
\]
where $\mathcal{W}$ denotes the set of treated cells and $\Omega$ denotes untreated (observed control) cells.
\end{assumption}
...
\[
\hat{\tau}_{\text{ATT}} = \frac{1}{|\mathcal{W}|} \sum_{(i,j,t) \in \mathcal{W}} \left( \mathcal{Y}_{ijt} - \hat{\mathcal{M}}_{ijt} \right),
\]
where $\hat{\mathcal{M}}$ is the tensor completion estimator. This result extends the matrix completion identification theory of \citet{athey2021matrix} to tensor structures; see also \citet{yuan2016tensor} for the statistical foundations.
\end{theorem}
...
Several software packages implement tensor completion. In Python, \texttt{tensorly} provides a general tensor decomposition library supporting CP, Tucker, and tensor completion with various backends (NumPy, PyTorch, TensorFlow), while \texttt{sktensor} handles tensor factorisation for sparse tensors and is particularly useful for large panels with many missing entries. In R, \texttt{rTensor} implements CP and Tucker decompositions with basic tensor operations and unfoldings, and \texttt{tensorBF} provides Bayesian tensor factorisation for uncertainty quantification. MATLAB users can employ the Tensor Toolbox (\texttt{tensor\_toolbox}), a comprehensive library for tensor computations including CP and Tucker with missing data support.
