...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!50!black,title=Algorithm: Soft-Impute for Matrix Completion]
\textbf{Input:} Observed entries $\{Y_{it}\}_{(i,t) \in \Omega}$, regularisation $\lambda$, rank $R$, tolerance $\epsilon$.

\textbf{Initialise:} Set $\hat{\mathbf{L}}^{(0)} = \mathbf{0}_{N \times T}$.

\textbf{For $k = 1, 2, \ldots$ until convergence:}
\begin{enumerate}
...
\textbf{Output:} Low-rank estimate $\hat{\mathbf{L}}$.
\end{tcolorbox}
...
\textbf{Decision tree for algorithm selection:}
\begin{enumerate}
