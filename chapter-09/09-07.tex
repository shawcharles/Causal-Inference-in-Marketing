...
Several software packages implement scalable matrix completion algorithms. In R, \texttt{softImpute} implements the soft-impute algorithm and handles matrices with millions of entries, \texttt{MatrixCompletion} provides nuclear norm minimisation, and \texttt{tensorBF} offers Bayesian tensor factorisation. In Python, \texttt{fancyimpute} provides multiple imputation algorithms including Soft-Impute, \texttt{scikit-learn}'s \texttt{NMF} handles matrix factorisation, \texttt{tensorly} implements tensor decomposition (CP and Tucker), and \texttt{implicit} offers ALS for large sparse matrices in recommender system contexts. Julia users can employ \texttt{LowRankModels.jl} for generalised low-rank models and \texttt{TensorToolbox.jl} for tensor decomposition.
