...
\paragraph{Marketing applications.}\begin{itemize}
\item \textbf{Binary outcomes:} Customer adoption or churn decisions with latent demand factors. Use logit likelihood with interactive fixed effects to model unobserved heterogeneity.
\item \textbf{Count outcomes:} Weekly purchase counts with latent category-time effects. Use Poisson likelihood with interactive fixed effects.
\item \textbf{Network models:} Social network effects with homophily in unobservables. The factor structure captures latent similarity.
\end{itemize}
...
\paragraph{Matrix completion + Difference-in-differences.} DiD estimates unit and time fixed effects. The hybrid applies MC to residuals:
\[
\tilde{Y}_{it} = Y_{it} - \hat{\alpha}_i - \hat{\gamma}_t, \quad \hat{Y}_{it}(0) = \hat{\alpha}_i + \hat{\gamma}_t + \hat{L}_{it}.
\]
...
\begin{itemize}
\item \textbf{Cross-validation:} Choose weights that minimise held-out prediction error on untreated cells.
\item \textbf{Inverse variance:} $\omega_m \propto 1/\text{Var}(\hat{\tau}_m)$, giving more weight to methods with smaller standard errors.
\item \textbf{Equal weights:} Simple averaging often performs well when methods are diverse.
\end{itemize}
Ensemble approaches are robust to model misspecification.
