...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!50!black,title=Algorithm: Online Matrix Completion with Subspace Tracking]
\textbf{Input:} Panel data $\{Y_{it}\}$, treatment indicators $\{W_{it}\}$, initial rank $R_0$, regularisation $\lambda$, smoothing $\alpha$.

\textbf{Initialise:} Estimate $\hat{\mathbf{L}}_1$ from first $T_0$ periods using standard MC. Set rank $R_1 = R_0$.

\textbf{For each time $t = T_0 + 1, \ldots, T$:}
\begin{enumerate}
...
\textbf{Output:} Time-varying estimates $\{\hat{\mathbf{L}}_t\}$, change points $\hat{\mathcal{B}}$, counterfactuals.
\end{tcolorbox}
