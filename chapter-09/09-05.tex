...
The low-rank assumption in Chapter~\ref{ch:factor} requires that the factor structure is stable over time. The rank $R$ (the number of factors) and the loadings $\lambda_i$ (unit sensitivities) are constant. This assumption is violated when structural breaks occur. A recession introduces new factors such as unemployment and credit constraints. A platform algorithm change alters user behaviour by creating new engagement patterns. The COVID-19 pandemic created a structural break in demand through remote work, supply chain disruptions, and consumption shifts.
...
\[
\hat{\mathbf{L}}_t = \arg\min_{\mathbf{L}} \sum_{s=1}^t \sum_{i=1}^N (1 - D_{is})(Y_{is} - L_{is})^2 + \lambda_t \|\mathbf{L}\|_* + \alpha \|\mathbf{L} - \hat{\mathbf{L}}_{t-1}\|_F^2,
\]
where:
\begin{enumerate}[(i)]
...
\paragraph{Input:} Panel data $\{Y_{it}\}$, treatment indicators $\{D_{it}\}$, initial rank $R_0$, regularisation $\lambda$, smoothing $\alpha$.
...
Several software packages support time-varying factor models and change-point detection. In Python, \texttt{ruptures} provides a change-point detection library supporting multiple algorithms including PELT and binary segmentation, \texttt{dynfact} implements dynamic factor models with time-varying loadings, and custom implementations using \texttt{scipy.optimize} can handle the online matrix completion objective. In R, \texttt{changepoint} provides change-point detection for time series using PELT and binary segmentation, \texttt{bfast} detects breaks in additive seasonal and trend components, \texttt{tseries} offers time series analysis including structural break tests, and \texttt{dfms} implements dynamic factor models with state-space representation. MATLAB users can employ GRASTA (Grassmannian Rank-One Update Subspace Tracking) for online subspace tracking or build custom implementations using the Optimization Toolbox.
...
\paragraph{Results.} The time-varying matrix completion estimator yields an ATT of 12\% (95\% CI: 8--16\%), compared to 8\% (95\% CI: 4--12\%) from standard matrix completion. The standard estimate is biased downward because it uses the pre-COVID structure ($R = 3$) for imputation, failing to capture the new factors.

\paragraph{Diagnostics.} Three checks validate the time-varying approach. First, change-point detection identifies mid-March 2020 as a structural break through a reconstruction error spike. Second, model fit improves after the break: pre-break $R^2 = 0.80$ versus post-break $R^2 = 0.85$ with $R = 5$. Third, sensitivity analysis shows the ATT is stable for $\alpha \in [0.1, 1.0]$.
