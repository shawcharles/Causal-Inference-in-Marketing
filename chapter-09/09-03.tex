...
Let $\mathcal{W}$ denote the set of treated cells. Treatment effects are distinguishable from outliers if:
\begin{enumerate}[(i)]
    \item The sparse component is estimated using only untreated cells: $\text{supp}(\hat{\mathbf{S}}) \cap \mathcal{W} = \emptyset$;
    \item Treatment effects satisfy $|\tau_{it}| \leq C_\tau \sigma$ for $(i,t) \in \mathcal{W}$;
...
Several software packages implement robust matrix completion. In Python, \texttt{sklearn.decomposition.PCA} with \texttt{svd\_solver='arpack'} provides standard PCA (not robust), while \texttt{rpca} implements Robust PCA using PCP with an ADMM solver, and \texttt{tensorly} includes robust tensor decomposition methods. In R, the \texttt{rpca} package provides Robust PCA with nuclear norm plus $\ell_1$ penalty using ADMM, \texttt{pcaMethods} offers various PCA methods including robust variants, and \texttt{robustbase} provides general robust statistical methods. MATLAB users can access Augmented Lagrange Multiplier (ALM) implementations from the authors of \citet{candes2011robust}.
...
\paragraph{Diagnostics.} Three checks validate the robust approach. First, the distribution of $\hat{\mathbf{S}}$ shows 95\% zero entries, with non-zero entries having mean absolute value 50 compared to mean sales of 100, confirming that outliers are sparse and substantial. Second, a placebo test randomly designates 5\% of untreated cells as artificial outliers, and the method correctly identifies 90\% of them. Third, sensitivity analysis shows ATT estimates are stable for $\lambda \in [0.05, 0.2]$.
