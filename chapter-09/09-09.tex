...
Several software packages support diagnostic analyses for matrix completion. In R, \texttt{RMTstat} provides random matrix theory tools for eigenvalue ratio tests, \texttt{influence.ME} offers influence diagnostics for mixed-effects models (adaptable to factor models), \texttt{car} provides Cook's distance, leverage plots, and DFFITS for regression, and \texttt{ggplot2} enables scree plots, residual distributions, and partial dependence plots. In Python, \texttt{statsmodels} provides influence diagnostics including Cook's distance and leverage, \texttt{sklearn.inspection} offers partial dependence plots and permutation importance, and \texttt{matplotlib} or \texttt{seaborn} enable scree plots and residual analysis.
