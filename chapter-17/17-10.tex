...
\paragraph{Pre-trend tests:} Use joint tests with cluster-robust or bootstrap inference. Report uniform confidence bands. Do not condition main inference on passing.

\paragraph{Placebo tests:} Use rank-based p-values with RMSPE ratios. Report sensitivity to placebo exclusion thresholds.

\paragraph{Small samples:} Use wild cluster bootstrap with Webb weights when $G < 50$. Use randomisation inference when $G < 10$ or when design-based justification is available.

\paragraph{Pre-test bias:} Report unconditional estimates. Use sensitivity analysis rather than binary pass/fail pre-trend tests. If conditioning, use honest inference methods.
\end{tcolorbox}
