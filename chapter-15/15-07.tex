...
Marketing panels often rely on data from platforms that constantly evolve, creating structural breaks that threaten parameter stability. A structural break occurs when model parameters shift at some point in time---the treatment effect, the baseline level, or the error variance changes. This is distinct from unit roots and other forms of nonstationarity discussed in Section~\ref{sec:regression-mechanics} and Appendix~\ref{app:stationarity-panels}. Structural breaks are the primary concern in short-$T$ marketing panels, where we cannot reliably test for unit roots but can often detect and model discrete parameter shifts.
