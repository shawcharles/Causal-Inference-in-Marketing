...
Estimate the hybrid method and compute pre-treatment RMSPE (Section~\ref{sec:hybrid-diagnostics}). Compare to standard SC, SDID, and DID to assess whether the hybrid improves fit. A useful threshold is RMSPE less than 5 to 10 per cent of outcome SD; improvement of at least 20 per cent over SC justifies the added complexity.
...
If pre-fit is poor (RMSPE exceeds 15 per cent of SD) or balance is weak (any SMD exceeds 0.3), revisit the donor pool, the predictor set, or the tuning parameters. This step is iterative: diagnostics inform refinements until acceptable fit is achieved or limitations are acknowledged.
...
Conduct leave-one-donor-out and leave-one-period-out sensitivity analyses, reporting the range of treatment effect estimates. If a single donor or period changes the estimate by more than 20 per cent, investigate and report this influence. Plot residuals over time and inspect for trends, seasonality, or outliers.
