...
For the campaign launch, cross-validation selects $\eta_w = 0.1$ and $\eta_\lambda = 0.05$. The unit weights concentrate on six markets with similar demographic profiles. The time weights down-weight the holiday quarter, when campaign markets experienced promotional activity unrelated to the treatment. The resulting estimate shows a 4.2\% lift in sales, with a standard error of 1.1\%---tighter than the DiD estimate (5.8\% with SE 2.3\%) and more plausible than the SC estimate (2.1\% with poor pre-treatment fit).
