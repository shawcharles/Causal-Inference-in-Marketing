...
A useful threshold is RMSPE less than 5 to 10 per cent of the outcome's standard deviation (Section~\ref{sec:hybrid-tuning}). If RMSPE exceeds this threshold, the hybrid model does not fit well enough to provide a credible counterfactual. RMSPE should be compared across methods (SC, ASCM, SDID, DID). If the hybrid achieves RMSPE at least 20 per cent lower than the simpler method, the additional complexity is likely justified. If the improvement is marginal, the simpler method is preferable for transparency.
...
Pre-treatment RMSPE for ASCM is 1.8 (outcome SD is 18), or 10 per cent of outcome SD. Standard SC achieves RMSPE 2.9 (16 per cent of SD). The 38 per cent reduction in RMSPE justifies the augmentation complexity. SDID achieves RMSPE 2.1 (12 per cent of SD), intermediate between ASCM and SC.
