...
Apply multiple methods when feasible. If the data support both DiD and synthetic control, run both. If results agree, confidence increases. If results diverge, investigate why. The divergence may reveal sensitivity to assumptions that a single method would hide. Section~\ref{sec:method-selection-outlook} discusses triangulation and multiverse analysis in detail.
...
Acknowledge when the question cannot be answered. Sometimes the data do not support causal inference. Selection is too severe, interference is too pervasive, structural instability is too pronounced. In these cases, the honest response is to say so. Descriptive analysis may still be valuable, but it should not be dressed up as causal. The credibility of the field depends on practitioners who know when to stop.

\subsection*{A Condensed Workflow}

For quick reference, here is the workflow in condensed form:

\begin{enumerate}
    \item \textbf{Define the estimand.} What effect, on what outcome, for what population, over what horizon?
    \item \textbf{Assess data.} Do you have the data the method requires?
    \item \textbf{Identify threats.} Selection? Interference? Structural instability? Measurement error?
    \item \textbf{Choose method.} Match data features and threats to method families (Section~\ref{sec:method-selection-outlook}).
    \item \textbf{Pre-register.} Commit to estimand, sample, method, diagnostics, inference.
    \item \textbf{Run diagnostics first.} Pre-trends, balance, overlap, fit---before seeing estimates.
    \item \textbf{Estimate.} Apply the method. Apply multiple methods if feasible.
    \item \textbf{Report transparently.} Estimand, assumptions, data, diagnostics, sensitivity, uncertainty.
    \item \textbf{If diagnostics fail:} Try alternatives, report bounds, or acknowledge the question cannot be answered.
\end{enumerate}
