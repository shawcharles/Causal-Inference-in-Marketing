...
\[
\text{ATT} = \mathbb{E}\big[Y_{it}(1) - Y_{it}(0) \mid W_{it} = 1\big].
\]
...
Modern staggered adoption designs motivate a further refinement. When units adopt treatment at different times $G_i \in \{1, \dots, T\}$, we define potential outcomes indexed by adoption time, $Y_{it}(a)$, denoting the outcome for unit $i$ in period $t$ if it had adopted treatment at time $a$. The realised outcome is $Y_{it} = Y_{it}(G_i)$.
...
\]
where weights $w_{gt}$ reflect relative sample sizes. Chapter~\ref{ch:did} discusses why traditional two-way fixed effects regressions fail to recover this convex combination when effects are heterogeneous.
...
\[
Y_{it}(\underline{w}^t) = Y_{it}(w_{it}).
\]
\end{assumption}
...
\addlinespace
ATT(g,t) & $\mathbb{E}[Y_{it}(g) - Y_{it}(\infty) \mid G_i=g]$ & Effect for cohort $g$ in period $t$ & Staggered rollouts; heterogeneity \\
\addlinespace
$\theta_k$ (Event-time) & $\mathbb{E}[Y_{i,G_i+k}(g) - Y_{i,G_i+k}(\infty) \mid G_i=g]$ & Effect $k$ periods post-adoption & Dynamic effects; pre-trends tests \\
\addlinespace
