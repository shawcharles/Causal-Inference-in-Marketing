...
\textbf{Regression with exposure.} \textit{Example: Retail panel with distance-weighted exposure to promoted stores.}
\begin{itemize}
...
\textbf{DiD with spillover groups.} \textit{Example: New store opening affecting nearby stores (spillover) vs distant stores (control).}
\begin{itemize}
...
\textbf{Two-stage design.} \textit{Example: Geo-experiment with varying treatment intensity across DMAs.}
\begin{itemize}
...
\textbf{Spatial Hausman--Taylor.} \textit{Example: Brand equity effect on sales with spatial correlation.}
\begin{itemize}
...
\textbf{SVR.} \textit{Example: Measuring cannibalization radius of flagship store opening.}
\begin{itemize}
