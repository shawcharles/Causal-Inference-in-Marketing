...
\paragraph{Spillover sign varies by context:}
\begin{itemize}
\item[$\bullet$] Geographic spillovers from promotions: \textbf{negative} (customers travel to treated stores).
\item[$\bullet$] Network spillovers from features: \textbf{positive} (content and social influence).
\item[$\bullet$] Competitive spillovers from advertising: \textbf{negative} (rivals respond).
\end{itemize}

\paragraph{Magnitude matters for decisions:}
\begin{itemize}
\item[$\bullet$] Small spillover adjustments may not change recommendations.
\item[$\bullet$] Large adjustments can substantially affect ROI calculations.
\item[$\bullet$] Non-linear effects (saturation) should inform optimal treatment intensity.
\end{itemize}

\paragraph{Estimation requires appropriate methods:}
\begin{itemize}
\item[$\bullet$] Geographic: Regression with exposure controls (Section~\ref{sec:estimating-spillovers}).
\item[$\bullet$] Network: Exposure bins for non-linearity (Section~\ref{sec:exposure-mappings}).
\item[$\bullet$] Competitive: Panel VAR for dynamic reactions (Section~\ref{sec:competition-saturation}).
\end{itemize}
\end{tcolorbox}
