...
The exposure mapping $f$ satisfies:
 ...
 \textbf{Boundedness:} $\sup_{\mathbf{w}, i} |f(\mathbf{w}, i)| < \infty$.

 \textbf{Locality:} There exists $\bar{s} < \infty$ such that $|\mathcal{S}_i| \leq \bar{s}$ for all $i$, where $\mathcal{S}_i = \{j : f(\mathbf{w}, i) \neq f(\mathbf{w}', i) \text{ for some } \mathbf{w}, \mathbf{w}' \text{ differing only in } w_j\}$.
 
 \textbf{Lipschitz:} If exposure depends on covariates $X_i$, then $|f(\mathbf{w}, i) - f(\mathbf{w}, i')| \leq L \|X_i - X_{i'}\|$ for some $L < \infty$.
...
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!75!black,title=Treatment Effects Under Interference]
\paragraph{Direct effect} (own treatment, holding exposure constant):
\[
\tau^{\text{direct}}(e) = \mathbb{E}[Y_{it}(1, e) - Y_{it}(0, e)].
\]

\paragraph{Spillover effect} (exposure change, holding own treatment constant):
\[
\tau^{\text{spillover}}(w, e, e') = \mathbb{E}[Y_{it}(w, e') - Y_{it}(w, e)].
\]

\paragraph{Total effect} (own treatment and exposure change):
\[
