...
\begin{tcolorbox}[colback=green!5!white,colframe=green!50!black,title=Box 10.2: Dynamic Effects Workflow Summary]

\paragraph{1. Define estimand:} Event-time effects, impulse responses, LRM, or half-life. Pre-specify window based on domain knowledge.

\paragraph{2. Map support:} Plot treated and control units for each lag; bin sparse tails.

\paragraph{3. Select estimator:} Event study (discrete), distributed-lag (continuous), or dynamic DiD (staggered).

\paragraph{4. Run pre-trend checks:} Estimate leads; interpret significant leads as anticipation or design violations.

\paragraph{5. Report dynamic profile:} Event-time plot with uniform confidence bands; LRM and half-life with bootstrap or delta-method SE.

\paragraph{6. Inference:} Cluster-robust SE; joint tests; multiple testing adjustments.

\paragraph{7. Diagnostics:} Follow protocol in Section~\ref{sec:dynamics-diagnostics}; placebo tests; sensitivity checks.
\end{tcolorbox}
...
\paragraph{1. Define Estimand:} Specify $\theta_k$, $\beta_s$, LRM, or half-life. Pre-specify window.

\paragraph{2. Map Support:} Plot treated/control per event time. Flag sparse lags. Bin or restrict.

\paragraph{3. Select Estimator:} Event study, distributed-lag, or dynamic DiD. Justify choice.

\paragraph{4. Specify Leads/Lags:} Define windows. Specify reference category. Document rationale.

\paragraph{5. Pre-Trend Checks:} Estimate leads. Plot with CIs. Joint F-test. Diagnose if rejected.

\paragraph{6. Choose Inference:} Cluster-robust, two-way, HAC, or wild bootstrap. Document choice.

\paragraph{7. Report Paths:} Plot event-time or impulse response. Report LRM and half-life with SE.

\paragraph{8. Sensitivity:} Vary window, binning, lag length, functional form, control group.

\paragraph{9. Document:} State assumptions. Discuss threats. Provide replication materials.
\end{tcolorbox}
