...
\paragraph{Binary treatment, common adoption timing:}
\begin{itemize}
\item Standard event study with two-way fixed effects.
\end{itemize}

\paragraph{Binary treatment, staggered adoption:}
\begin{itemize}
...
\paragraph{Continuous treatment (advertising GRPs, discount depth):}
\begin{itemize}
...
\paragraph{Too short:} Truncates impulse response; biases LRM downward.

\paragraph{Too long:} Includes zero-effect lags; inflates standard errors; overfitting.
...
\paragraph{Caution:} If state dependence is spurious (driven by serial correlation in unobservables), the DPDM biases treatment effects. See \citet{arellano2003panel} for foundations.
\end{tcolorbox}
