...
\end{enumerate}
The untreated potential outcome is $Y_{it}(0) \equiv Y_{it}(\underline{w}^{\text{never}})$, and the treated potential outcome under adoption at $g$ is $Y_{it}^g \equiv Y_{it}(\underline{w}^{\text{adopt}}(g))$.
\end{definition}
...
\[
Y_{it} = \sum_{s=0}^{\bar{L}} \beta_s W_{i,t-s} + \alpha_i + \gamma_t + \varepsilon_{it},
\]
where $W_{it} \in \mathbb{R}$ is treatment intensity, $\bar{L}$ is the maximum lag, $\alpha_i$ are unit fixed effects, $\gamma_t$ are time fixed effects, and $\varepsilon_{it}$ are idiosyncratic errors.
\begin{enumerate}[(i)]
...
\textbf{Setting:} Weekly advertising campaign with estimated impulse response $\beta_s = 100 \times 0.7^s$ (geometric decay with $\delta = 0.7$ and $\beta_0 = 100$ units of incremental sales).
...
\textbf{Policy implication:} With a 2-week half-life, weekly pulsing maintains awareness better than monthly pulsing. If LRM = 333 and cost-per-campaign = \$10,000, the cost-per-incremental-unit is \$30.
\end{tcolorbox}
...
\midrule
Event-time effect $\theta_k$ & $\mathbb{E}[Y_{i,g+k}^g - Y_{i,g+k}(0) \mid G_i = g]$ & Effect $k$ periods after adoption \\
\addlinespace
