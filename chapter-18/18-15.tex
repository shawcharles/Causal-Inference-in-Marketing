...
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!75!black,title=Box 18.18: Reporting Standards Checklist]
\textbf{1. Pre-registration:} Document the estimand, design, and primary specification before analysis. Register with a timestamp (internal wiki, OSF, or email to stakeholders).

\textbf{2. Estimand clarity:} State the causal quantity (ATT, ATE, GATE) and its business interpretation. Avoid vague language like "the effect of X on Y."

\textbf{3. Assumption disclosure:} List the identification assumptions and discuss their plausibility in context. Do not bury assumptions in footnotes.

\textbf{4. Diagnostic reporting:} Include pre-trend tests, balance tables, fit statistics, and placebo results (Chapter~\ref{ch:design-diagnostics}). Report p-values and effect sizes, not just pass/fail.

\textbf{5. Sensitivity analysis:} Report specification curves or robustness to alternative designs (Section~\ref{sec:sensitivity-analyses}). Show what would change the conclusion.

\textbf{6. Confidence intervals:} Report uncertainty using appropriate inference (cluster-robust, bootstrap, permutation). Never report point estimates without uncertainty.

\textbf{7. Business translation:} Convert the ATT to actionable metrics (iROAS, incremental CLV, elasticity). Include confidence intervals on business metrics.
\end{tcolorbox}
...
\textbf{1. Executive Summary} (1 page)
\begin{itemize}
...
\textbf{8. Business Implications} (1 page)
\begin{itemize}
...
\textbf{Appendix:} Technical details, reproducible code (with package versions), data dictionary, and pre-registration documentation
\end{tcolorbox}
